...
function useOneDrive() {
  ...
  var signIn = function() { setShowForm(true); setErr(""); };

  var doConnect = async function(clientId, tenantId) {
    setShowForm(false);
    ...
    try {
      ...
      var r = await msal.loginPopup({scopes:["Files.ReadWrite","User.Read"]});
      var tok = r.accessToken;
      var name = r.account.name || r.account.username;
      setStatus("verifying");
      var fr = await fetch("https://graph.microsoft.com/v1.0/me/drive/root:/Gamber_valley_MIS_26-27.xlsx",
        {headers:{Authorization:"Bearer "+tok}});
      if (!fr.ok) throw new Error("File 'Gamber_valley_MIS_26-27.xlsx' not found in your OneDrive root.");
      var file = await fr.json();
      var sr = await fetch("https://graph.microsoft.com/v1.0/me/drive/items/"+file.id+"/workbook/worksheets/MPR/usedRange",
        {headers:{Authorization:"Bearer "+tok}});
      var sheet = await sr.json();
      var rows = (sheet.values && sheet.values[3] ? sheet.values[3].slice(2) : []).filter(function(v){return typeof v==="number"&&v>0;});
      ...
      setStatus("error"); setErr(e.message || "Connection failed.");
    }
  };
...
function CredentialModal({ onConnect, onCancel, err }) {
  var [cid, setCid] = useState("");
  var [tid, setTid] = useState("");

  var inp = {
    width:"100%", border:"1px solid "+T.bdr2, borderRadius:6,
    padding:"9px 12px", fontSize:13, outline:"none", color:T.tx1,
    fontFamily:T.sans, boxSizing:"border-box",
  };
  ...
  return (
    <div style={{position:"fixed",inset:0,zIndex:600,display:"flex",alignItems:"center",
      ...
      onClick={onCancel}>
      <div onClick={function(e){e.stopPropagation();}}
        style={{width:"min(400px,92vw)",background:T.white,borderRadius:12,
          boxShadow:"0 20px 60px rgba(0,0,0,.2)",overflow:"hidden"}}>
        <div style={{padding:"15px 18px",borderBottom:"1px solid "+T.bdr}}>
          <div style={{fontSize:14,fontWeight:700,color:T.tx1}}>Connect to OneDrive</div>
          <div style={{fontSize:12,color:T.tx3,marginTop:3,lineHeight:1.5}}>
            Enter the Azure credentials from your admin (one-time only — stored in your session).
          </div>
        </div>
        <div style={{padding:18,display:"flex",flexDirection:"column",gap:12}}>
          <div>
            <label style={{fontSize:11,fontWeight:600,color:T.tx2,display:"block",
              marginBottom:5,textTransform:"uppercase",letterSpacing:.3}}>
              Azure App Client ID
            </label>
            <input value={cid} onChange={function(e){setCid(e.target.value);}}
          ...
          <div>
            ...
            <input value={tid} onChange={function(e){setTid(e.target.value);}}
              placeholder="xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"
              style={inp}/>
          </div>
          {err ? (
            <div style={{background:T.redSoft,border:"1px solid rgba(229,57,53,.3)",
              borderRadius:6,padding:"8px 12px",fontSize:12,color:T.red}}>
              {err}
            </div>
          ) : null}
          <div style={{background:T.blueSoft,borderRadius:6,padding:"10px 12px",fontSize:12,color:T.tx2,lineHeight:1.6}}>
            <strong style={{color:T.blue}}>Where to find these:</strong> portal.azure.com
            → App registrations → your app → Overview page.
            Your admin sets this up once — employees only need to enter it once per device.
          </div>
        </div>
        <div style={{padding:"12px 18px",borderTop:"1px solid "+T.bdr,display:"flex",gap:8}}>
          ...
          <button onClick={function(){if(cid.trim()&&tid.trim())onConnect(cid.trim(),tid.trim());}}
            disabled={!cid.trim()||!tid.trim()}
            style={{flex:2,padding:"9px",border:"none",borderRadius:6,background:T.blue,
              color:"#fff",cursor:"pointer",fontSize:13,fontWeight:600,
              opacity:!cid.trim()||!tid.trim()?0.5:1}}>
          ...
          {!od.token ? (
            <div>
              <p style={{fontSize:13,color:T.tx2,lineHeight:1.6,marginBottom:12}}>
                Sign in with your <strong>company Microsoft 365 account</strong> to load live
                data from your OneDrive Excel file.
              </p>
...
function AdminGate({ onUnlocked, onCancel }) {
  ...
  var submit = async function() {
    ...
    try {
      ...
      if (pw === "admin2026") { sessionStorage.setItem("adm","1"); onUnlocked(); }
...
function ModulePage({ id, isAdmin }) {
  ...
  var fn = configs[id];
  return fn ? fn() : <div style={{color:T.tx3,padding:20}}>Module not found.</div>;
}
...
export default function App() {
  ...
  return (
    <div style={{display:"flex",minHeight:"100vh",background:T.bg,
      ...
      {od.showForm ? (
        <CredentialModal err={od.err}
          onConnect={od.doConnect}
          onCancel={function(){od.setShowForm(false);}}/>
      ) : null}
